...
//	{MENU_ENTRY_SUBMENU,"RGB Scaling \x10",MENU_ACTION(rgbmenu)},
	...
	{MENU_ENTRY_TOGGLE,"Sid 8580",MENU_ACTION(1)},
	{MENU_ENTRY_TOGGLE,"Color CePCero",MENU_ACTION(2)},
	{MENU_ENTRY_TOGGLE,"Ntsc",MENU_ACTION(3)},
...
//	{MENU_ENTRY_CALLBACK,"Animate",MENU_ACTION(&TriggerEffect)},
	{MENU_ENTRY_CALLBACK,"Cargar Disco \x10",MENU_ACTION(&FileSelector_Show)},
...
{
	{MENU_ENTRY_SUBMENU,"Carga de DSK Fallida",MENU_ACTION(loadfailed)},
...
static int LoadROM(const char *filename)
{
	...
	HW_HOST(REG_HOST_CONTROL)=HOST_CONTROL_LOADROM; //Inicai Señal de carga
	...
	SuperDelay();
	SwichRomSignal(0); //Envia Final señal de carga
...
//	LoadROM("PIC1    RAW");
	...
	while(1)
	{
		...
		if(MENU_TOGGLE_VALUES&2)
			dipsw|=2;	// Add in the Voices bit
		if(MENU_TOGGLE_VALUES&4)
			dipsw|=4;	// Add in the Color bit
